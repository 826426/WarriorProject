...
{
	GetHeroUIComponenttFromActorInfo()->OnStoneInteracted.Broadcast(true);
...
{
	GetHeroUIComponenttFromActorInfo()->OnStoneInteracted.Broadcast(false);
...
void UHeroGameplayAbility_PickUpStones::CollectStones()
{
	...
	for (const FHitResult& TraceHit : TraceHits) {
		if (AWarrioStoneBase* FoundStone = Cast<AWarrioStoneBase>(TraceHit.GetActor())) {
...
void UHeroGameplayAbility_PickUpStones::ConsumeStones()
{
	...
	for (AWarrioStoneBase* CollectedStone : CollectedStones) {
		if (CollectedStone)
		{

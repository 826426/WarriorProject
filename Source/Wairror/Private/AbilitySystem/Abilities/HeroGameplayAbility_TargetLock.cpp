...
void UHeroGameplayAbility_TargetLock::OnTargetLockTick(float DeltaTime)
{
	if (!CurrentLockedActor ||
		UWarriorFunctionLibrary::NativeDoesActorHaveTag(CurrentLockedActor, WairroGamePlayerTags::Shared_Status_Dead) ||
		UWarriorFunctionLibrary::NativeDoesActorHaveTag(GetHeroCharacterFromActorInfo(), WairroGamePlayerTags::Shared_Status_Dead)
		) {
	...
	const bool bShouldOverrideRotation =
		!UWarriorFunctionLibrary::NativeDoesActorHaveTag(GetHeroCharacterFromActorInfo(), WairroGamePlayerTags::Player_Status_Rolling)
		&&
		!UWarriorFunctionLibrary::NativeDoesActorHaveTag(GetHeroCharacterFromActorInfo(), WairroGamePlayerTags::Player_Status_Blocking);
...
void UHeroGameplayAbility_TargetLock::SwitchTarget(const FGameplayTag& InSwitchDirectionTag)
{
	...
	if (InSwitchDirectionTag == WairroGamePlayerTags::Player_Event_SwitchTarget_Left) {
...
void UHeroGameplayAbility_TargetLock::GetAvailableActorsToLock()
{
	...
	for (const FHitResult& TracHit : BoxTraceHits) {
		if (AActor* HitActor = TracHit.GetActor()) {
...
{
	float ClosesDistance = 0.0f;
	return UGameplayStatics::FindNearestActor(GetHeroCharacterFromActorInfo()->GetActorLocation(), InAvailableActors, ClosesDistance);
}
